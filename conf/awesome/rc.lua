...
require("ensure") -- ensure there are no configuration errors (:
...
local function set_volume(action)
	if action == "toggle" then
		local command = string.format("pactl set-sink-mute %s %s", 0, action)
		os.execute(command)
		-- beautiful.volume.update()
	else
		local command = string.format("pactl set-sink-volume %s %s", 0, action)
		os.execute(command)
		-- beautiful.volume.update()
	end
end
