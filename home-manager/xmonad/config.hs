...
brightnessctl c = "brightnessctl s" <> c

maimcopy = "maim -s | xclip -selection clipboard -t image/png && notify-send \"Screenshot\" \"Copied to Clipboard\" -i flameshot"
maimsave = "maim -s $HOME/Desktop/$(date +%Y-%m-%d_%H-%M-%S).png && notify-send \"Screenshot\" \"Saved to Desktop\" -i flameshot"
...
myKeys conf@XConfig {XMonad.modMask = modm} =
  keySet "Applications"
    [ key "Slack"         (modm                , xK_F2      ) $ spawnOn commsWs "slack"
    ] ^++^
  keySet "Audio"
    [ key "Mute"          (0, xF86XK_AudioMute              ) $ spawn "pactl set-sink-mute 0 toggle"
    , key "Lower volume"  (0, xF86XK_AudioLowerVolume       ) $ spawn "pactl set-sink-volume 0 -5%"
    , key "Raise volume"  (0, xF86XK_AudioRaiseVolume       ) $ spawn "pactl set-sink-volume 0 +5%"
  ...
  keySet "Brightness"
    [ key "Increase brightness" (0, xF86XK_MonBrightnessUp)   $ spawn $ brightnessctl "+10%"
    , key "Lower brightness"    (0, xF86XK_MonBrightnessDown) $ spawn $ brightnessctl "10-%"
    ] ^++^
  keySet "Launchers"
  ...
  keySet "PrintScreen"
    [ key "Print area to clipboard" (0,    xK_Print) $ spawn maimcopy
    , key "Print area to Desktop"   (modm, xK_Print) $ spawn maimsave
    ] ^++^
  keySet "Layouts"
...
myLayout =
  avoidStruts
    ...
    . etcLayout
    . eightLayout
    . nineLayout $ (tiled ||| Mirror tiled ||| column3 ||| full)
   where
     ...
     wwwLayout = onWorkspace wwwWs (tiled ||| full)
     devLayout = onWorkspace devWs (Mirror tiled ||| full)
     meetLayout = onWorkspace meetWs (tiled ||| full)
     commsLayout = onWorkspace commsWs (tiled ||| full)
     mediaLayout = onWorkspace mediaWs (tiled ||| full)
     sysLayout = onWorkspace sysWs (tiled ||| full)
     etcLayout = onWorkspace etcWs (tiled ||| full)
     eightLayout = onWorkspace eightWs (tiled ||| full)
     nineLayout = onWorkspace nineWs (tiled ||| full)
...
myWS :: [WorkspaceId]
myWS = [wwwWs, devWs, meetWs, commsWs, mediaWs, sysWs, etcWs, eightWs, nineWs]
