...
clipboardy = spawn "rofi -modi \"\63053 :greenclip print\" -show \"\63053 \" -run-command '{cmd}' -theme $HOME/.config/rofi/launcher/style.rasi"

centerlaunch = spawn "exec $HOME/.local/bin/eww open-many blur_full weather profile quote search_full incognito-icon vpn-icon home_dir screenshot power_full reboot_full lock_full logout_full suspend_full"
sidebarlaunch = spawn "exec $HOME/.local/bin/eww open-many weather_side time_side smol_calendar player_side sys_side sliders_side"
ewwclose = spawn "exec $HOME/.local/bin/eww close-all"
maimcopy = spawn "maim -s | xclip -selection clipboard -t image/png && notify-send \"Screenshot\" \"Copied to Clipboard\" -i flameshot"
maimsave = spawn "maim -s $HOME/Desktop/$(date +%Y-%m-%d_%H-%M-%S).png && notify-send \"Screenshot\" \"Saved to Desktop\" -i flameshot"
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    , ((modm .|. shiftMask, xK_g), sendMessage $ setGaps [(L,2), (R,2), (U,10), (D,2)]) -- reset the GapSpec
...
myStartupHook = do
  ...
  spawnOnce "feh --bg-scale ~/.local/share/wallpapers/yosemite-lowpoly.jpg"
  spawnOnce "kill -9 $(pidof picom) && picom --experimental-backends --config $HOME/.config/picom.conf"
...
defaults = def {
      ...
      -- hooks, layouts
        ...
        layoutHook = gaps [(L,2), (R,2), (U,2), (D,2)] $ spacingRaw True (Border 0 2 0 0) True (Border 0 2 0 0) True $ smartBorders $ myLayout,
